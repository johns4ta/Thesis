...
DATABASE = "httparchive_20131015.sq3" #Database tables are located in.
PARSED_TABLE = "prequestsmobile" #Table created when columns are stripped from original table.
	
def getKey(item):
	return int(item[0])
	
def getTotalBytes(sorted_info):
	sum = 0
	for tup in sorted_info:
		sum += tup[1]
	return sum
...
def startReading():
	...
	with open(OUTPUT_FILE, 'wb') as csvfile:
		...
		with con:
			...
			cur.execute("SELECT expAge,respSize FROM " + PARSED_TABLE + ";")
			info = cur.fetchall()

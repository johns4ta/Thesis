...
FILES_PATH = '/home/johns4ta/Sim/Simulated Data Sim/simData'
OUTPUT_FILE = 'graphing_info.csv'
	...
	for x in range(len(time_intervals)):
		...
		if int(data[1]) <  time_intervals[x]:
			if int(bytes_total) == 0 or int(req_total) == 0:
				totBytes_proportions[x].append(1) #No bytes or no requests, just append 1 (prevent div. by 0)
				totRequests_proportions[x].append(1) #No bytes or no requests, just append 1 (prevent div. by 0)
			else:
				totBytes_proportions[x].append(float(data[2])/float(bytes_total)) #Append proportion of bytes that haven't expired
				totRequests_proportions[x].append(float(data[3])/float(req_total)) #Append proportion of bytes that haven't expired
			break;
